  ...
  methods: {
    ...
    option(val) {
      if (val == "111") {
        ...
        this.$store
          .dispatch("applyVacation/applyVacationToEdit", params)
          .then(res => {
        ...
        this.$confirm("是否确认删除?", "提示", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("applyVacation/applyVacationDelete", params.join(","))
              .then(res => {
                if (res.status == "0") {
        ...
        if (this.seletedItem[0].applyStatus !== "4") {
          return this.$message.warning("此条数据状态不为申请成功,不能销假");
        }
        if (this.seletedItem[0].returnTime == "" || this.seletedItem[0].returnTime == null) {
          this.$refs.addOrModify.showDialog(val, this.seletedItem);
       }else{
