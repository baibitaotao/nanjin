<template>
  <div class="page">
    ...
    <report-back-controller ref="reportBackController"></report-back-controller>
...
import reportBackController from "./dialog/reportBackController";
...
export default {
  ...
  components: { myTable, addOrModify, reportBackController, detail },
  data() {
    return {
      seletedItem: [],
      queryParams: {
  ...
  methods: {
    ...
    option(val) {
      if (val == "111") {
        ...
        this.$store.dispatch("applyOut/applyOutToEdit", params).then(res => {
        ...
        this.$confirm("是否确认删除?", "提示", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("applyOut/applyOutDelete", params.join(","))
              .then(res => {
                if (res.status == "0") {
        ...
        if (this.seletedItem[0].applyStatus !== "4") {
          return this.$message.warning(
            "此条数据状态不为申请成功，不能进行此操作"
          );
        }
        if (
          this.seletedItem[0].returnTime == null ||
          this.seletedItem[0].returnTime == ""
        ) {
          this.$refs.addOrModify.showDialog(val, this.seletedItem);
        } else {
          return this.$message.warning("已归来!");
        }
      } else if (val === "115") {
