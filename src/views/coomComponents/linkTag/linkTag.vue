      ...
      <el-col ref="menu" :span="24" :style="{maxWidth:cmaxWidth+'px'}">
        <div class="ul-contaier">
          <ul class="tag-nav" :class="{activeHeight:isActiveH}">
            <li v-for="(item, index) in options" :key="item.value" class="navItem"
                :class="item.checked === true?'activeClass':''" @click="changeTags(index)">
  ...
  export default {
    ...
    props: {
      ...
      contextPath: {// 请求地址
        type: String,
        default: '/oa-web/',
        // default: 'http://172.29.201.69:8072/ei-web/'
      },
      parentId: {// 项目id
        type: String,
        default: ''
    ...
    methods: {
      getCodeByParentId(parentId) { // 获取options label data 数据
        ...
        //  const URL = `/ei-web/commonTCode/getCodeByParentId?parentId=${parentId}`
        axios({
          url: URL,
          method: 'get'
        }).then((response) => {
