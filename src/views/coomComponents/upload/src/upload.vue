  ...
  <div class="upload-box">
    <div v-if="hiddenUpload" class="fileView">
      <h3 v-for="item in fileList" :key="item.uid">
        <a
          :href="contextPath + 'attachController/download?fileId=' + item.uid"
          target="_blank"
        >{{ item.name }}</a>
      </h3>
    </div>
    <div v-if="!hiddenUpload" class="el-upload-box">
      ...
      >
        <el-button :class="{active:hiddenUpload}" :size="size" type="primary">{{ uploadTitle }}</el-button>
...
// 实现组件数据双向绑定
// this.$emit('update-value', string_id)
//   model: {
//       prop: 'value',
//       event: 'update-value'
//   },
export default {
  ...
  props: {
    ...
    contextPath: {
      ...
      type: String,
      default: "/oa-web/"
      // default: "http://172.29.201.230:9090/oa-web/"
    },
    acceptFormat: {
      type: String,
      default:
        ".jpg,.jpeg,.png,.gif,.bmp,.pdf,.JPG,.JPEG,.PBG,.GIF,.BMP,.PDF,.rp,.vsd"
    }
  },
  data() {
    return {
  ...
  created() {
    setTimeout(() => {
      this.$nextTick(() => {
        this.updatePath =
          this.contextPath + "attachController/upload?fieldToken=" + this.value;
          this.getFileData();
      });
    }, 500);
  },
  methods: {
    ...
    handleSuccess(response, file, fileList) {
      ...
      this.updatePath =
        this.contextPath + "attachController/upload?fieldToken=" + fieldToken;
    ...
    handlePreview(file) {
      // 点击文件列表中已上传的文件时
      window.open(
        this.contextPath + "attachController/download?fileId=" + file.uid
      );
    }
  }
};
