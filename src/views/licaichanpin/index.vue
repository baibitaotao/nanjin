  ...
  <div class="page">
    <div>
      ...
      <transition name="el-zoom-in-top">
        <div class="conditions" v-show="isShowCondition"></div>
      </transition>
    ...
    <div class="bottom">
      ...
      <my-table
        ...
        ref="myTable"
        @transferSelectItem="transferSelectItem"
        @shuhu="shuhu"
        @xiangqing="xiangqing"
      ></my-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    shuhu(val) {
      let arr = [];
      arr.push(val);
      this.$refs.addOrModify.showDialog("200", arr);
    },
    xiangqing(val) {
      let params = { applyNo: val.applyNo };
      this.$store
        .dispatch("licaichanpin/applyFinancialDetail", params)
        .then(res => {
          if (res.status === "0") {
            this.$refs.detail.showDialog("200", res.data);
          }
        });
    },
    option(val) {
      if (val == "111") {
        ...
        this.$store
          .dispatch("licaichanpin/applyFinancialToEdit", params)
          .then(res => {
            ...
            this.fixedData.buyNo = res.data.buyNo;
        ...
        this.$confirm("是否确认删除?", "提示", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("licaichanpin/applyFinancialDelete", params.join(","))
              .then(res => {
                if (res.status == "0") {
        ...
        // this.$refs.detail.showDialog(val, this.seletedItem);
        this.xiangqing(this.seletedItem[0])
      }
    },
    getTableList() {
