  ...
  <div class="page">
    <div>
      <div>
        ...
        <el-input v-model="queryParams.keyWord" placeholder="请输入会议名称搜索" class="input-with-select" size="small"></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      allItem: 0,
      buttons: [
        { code: "111", name: "会议申请", btnScope: "normal" },
  ...
  methods: {
    ...
    option(val) {
      if (val == "111") {
        ...
        this.$store
          .dispatch("meetingCost/meetingCostToEdit", params)
          .then(res => {
        ...
        this.$confirm("是否确认删除?", "提示", {
          ...
          .then(() => {
            ...
            // let data = {applyNo:params.join(",")}
            this.$store
              .dispatch("meetingCost/meetingCostDetail",params.join(","))
              .then(res => {
                if (res.status == "0") {
