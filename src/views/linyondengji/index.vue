  ...
  <div class="page">
    <div>
      <div>
        ...
        <el-input v-model="queryParams.keyWord" placeholder="请输入申请单号/物品种类/物品编号搜索" class="input-with-select" size="small"></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      allItem: 0,
      buttons: [
        { code: "111", name: "领用", btnScope: "normal" },
        { code: "116", name: "归还", btnScope: "single" },
  ...
  methods: {
    ...
    option(val) {
      if (val == "111") {
        ...
        this.$store
          .dispatch("linyondengji/assetsUseToEdit", params)
          .then(res => {
        ...
        this.$confirm("是否确认删除?", "提示", {
          ...
          .then(() => {
            ...
            this.seletedItem.forEach(item => {
              params.push(item.useId);
            });
            this.$store
              .dispatch("linyondengji/assetsUseDelete", params.join(","))
              .then(res => {
                if (res.status == "0") {
        ...
        this.$refs.detail.showDialog(this.seletedItem);
         ...
         if (this.seletedItem[0].applyStatus !== "4") {
          return this.$message.warning("此条数据不能进行归还操作!");
        }
        this.$refs.addOrModify.showDialog(val,this.seletedItem);
      }
    },
