  ...
  <div class="page">
    <div>
      <div>
        ...
        <el-input v-model="queryParams.keyWord" placeholder="请输入收款单位搜索" class="input-with-select" size="small"></el-input>
      ...
      <transition name="el-zoom-in-top">
        <div class="conditions" v-show="isShowCondition">
          <div style="display:flex">
            <span class="tagText">付款方式</span>
            <link-tag parentId="11012" :selectType="1" v-model="queryParams.paymentType"></link-tag>
          </div>

          <div style="display:flex">
            <span class="tagText">用途</span>
            <link-tag parentId="11013" :selectType="1" v-model="queryParams.useType"></link-tag>
          </div>
          <div>
...
import linkTag from "../coomComponents/linkTag/src/linkTag";
// import linkTag from "../coomComponents/linkTag/linkTag";
...
export default {
  ...
  components: { myTable, addOrModify, linkTag, detail },
  data() {
    return {
      seletedItem: [],
      queryParams: {
  ...
  methods: {
    ...
    option(val) {
      if (val == "111") {
        ...
        this.$store
          .dispatch("applyBorrow/applyBorrowToEdit", params)
          .then(res => {
            ...
            this.fixedData.applyTimeLong = res.data.applyTimeLong;
        ...
        this.$confirm("是否确认删除?", "提示", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("applyBorrow/applyBorrowDelete", params.join(","))
              .then(res => {
                if (res.status == "0") {
