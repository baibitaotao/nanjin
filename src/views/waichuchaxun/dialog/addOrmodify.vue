<template>
  <div class="addOrModify">
    <el-dialog title="外出申请新增" :visible.sync="dialogFormVisible">
      <el-dialog width="80%" title="选择人员" :visible.sync="innerVisible" append-to-body>
        <select-person v-if="innerVisible" v-model="userId" :contextPath="'/oa-web'" @update-value="getselectItemMP"></select-person>
      </el-dialog>
          ...
          <!-- <el-col :span="12">
            <div class="grid-content bg-purple">
              <el-form-item label="会议费用" prop="meetingAmount">
                <el-input v-model="ruleForm.meetingAmount" type="number"></el-input>
              </el-form-item>
            </div>
          </el-col>-->
        </el-row>
          ...
          <el-col :span="24">
            <div class="grid-content bg-purple fujian">
              <el-form-item label="附件">
                <up-load v-if="dialogFormVisible" :contextPath="'/oa-web/'" uploadTitle="点击上传附件" v-model="ruleForm.attachment" ></up-load>
              </el-form-item>
            </div>
          </el-col>
        </el-row>
...
export default {
  ...
  watch: {
     userId(val,oldval){
       if(this.WhichClick == 'observerPerson'){this.formId.observerPerson = val }
       else if(this.WhichClick == 'recordPerson'){this.formId.recordPerson = val}
       else if(this.WhichClick == 'meetingHost'){this.formId.meetingHost = val}
       else if(this.WhichClick == 'meetingPerson'){this.formId.meetingPerson = val}
     } 
  },
  data() {
    return {
